...
from config import config_func_unetpp
...
def main(putanja_train, putanja_val,p_index):
    ...
    tmp = get_args('train','UNet++')
    globals().update(tmp)
    base_folder_path = os.getcwd()
    base_folder_path = base_folder_path.replace("\\", "/")

    ime_foldera_za_upis,logs_path, save_model_path = pretraining_prints(p_index,lr,stepovi,lambda_parametri,batch_size,loss_type,net_type)

    ####################
    ### data loading ###
    ####################
    
    train_loader, valid_loader = data_loading(ime_foldera_za_upis,putanja_train,putanja_val,binary,p_index,net_type)
    ...
    plt.show()
    plt.savefig("./model_loss_training_unet++_"+str(epochs)+"_masked_epochs_lr_"+str(lr)+".png")

    np.save(logs_path + "/all_train_losses_"+str(p_index)+".npy", all_train_losses)
    np.save(logs_path + "/all_validation_losses_"+str(p_index)+".npy", all_validation_losses)
    ...
    if do_testing:
        ...
        test_loader = AgroVisionDataLoader(img_size, numpy_test_path, img_data_format, p_index, # umesto p_indexa shuffle parametar stoji po difoltu ,
        ...
        IOU = run_testing(segmentation_net, test_loader, ime_foldera_za_upis, device, num_channels_lab, classes_labels,classes_labels2,
                     criterion_1, loss_type, tb, zscore,net_type)
...
if __name__ == '__main__':
    config_func_unetpp(server = True,epochs=40)
    # lr = [1e-2,1e-3,1e-4]
    lr = [1e-3] 
    lambda_parametar = [1]
    stepovi_arr = [5]
        ...
        for step_index in range(len(stepovi_arr)): # petlja kojom ispitujemo kako se trening menja za razlicite korake promene lr-a, step = broj iteracija nakon kojeg ce se odraditi scheduler.step(loss)
            for lambd_index in range(len(lambda_parametar)):  # petlja kojom ispitujemo kako se trening menja za razlicite labmda parametre kojim mnozimo lr kada dodje do ispunjavanja uslova za scheduler.step(loss)
                for lr_index in range(len(lr)): # petlja kojom ispitujemo kako se trening menja za razlicite lr-ove
                    main(r"/home/stefanovicd/DeepSleep/agrovision/DetekcijaBorovnica/leave_one_out/Experiment5/trening_set_masked/img",r"/home/stefanovicd/DeepSleep/agrovision/DetekcijaBorovnica/leave_one_out/Experiment5/validation_set/img",p_index)
