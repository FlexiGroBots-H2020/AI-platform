...
def postprocessing_block(GeoTiff,full_geotiff,y0,y1,x0,x1,final_mask,
                         geotransform, projection, load_pred_pth,save_final_GeoTiff_pth,
                         save_final_colored_GeoTiff_pth,save_final_shp_pth,deltas):
    ...
    # create the 3-band raster file (left_delta,top_delta,right_delta,bottom_delta)
    np.save(os.path.join(save_final_GeoTiff_pth,"detected_rows_mask.npy"),Final_prediction[deltas[1]:-deltas[3],deltas[0]:-deltas[2]]*255)
...
def main(model_path, net_type, device, input_files_type=None):
    ...
    deltas = (left_delta,top_delta,right_delta,bottom_delta)
    ...
    postprocessing_block(stacked_geotiffs_npy,full_stacked_geotiffs,
                         y0_new,y1_new,x0_new,x1_new,final_mask,
                         geo_transform,wkt_raster_proj,load_pred_data_pth,
                         save_final_GeoTiff_pth, save_final_colored_GeoTiff_pth,
                         save_final_shp_pth,deltas)
