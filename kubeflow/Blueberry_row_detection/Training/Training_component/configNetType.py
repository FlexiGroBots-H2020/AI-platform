...
### Hyperparameters ###
#######################
def config_func_nettype(server=True,net_type = "UNet3"):
    ...
    classes_labels3 = ['Model', 'Learning_rate', 'optimizer', 'lambda', 'Step', 'Num_epchs', 'Batch_size', 'Background', 'Blueberries']
    ...
    net_type = net_type      # Indicates Architecture that we want to use: UNet3, Unet_orig,UNet3_modified...
    ...
    save_checkpoint_freq = 5    #Initial frequency parameter for model saving

    if save_checkpoint_freq < 10:  # Further updating saving frequency parameter
        save_checkpoint_freq = 5
    if save_checkpoint_freq >= 10:
    ...
    dictionary = {
        ...
        "epoch_model_best_save" : epoch_model_best_save,
    ...
    with open("config_nettype_"+net_type+".json", "w") as outfile:
