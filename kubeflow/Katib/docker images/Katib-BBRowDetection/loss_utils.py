...
def loss_calc(loss_type ,criterion ,model_output ,target_var ,num_channels_lab = 2, use_mask = True): ### num_channels_lab = 2 u slucaju kada imamo 2 klase, bg i fg, Za Saletov slucaj to ce biti 7
    ...
    if loss_type == "bce":                                                       ### proslediti
        ...
        if use_mask:
            loss = loss[cfg.mask_train.unsqueeze(1).repeat(1,num_channels_lab,1,1)]
            loss = loss.mean()
            return loss
        else:
            return loss
        ...
        if use_mask:
            loss = torch.multiply(loss, cfg.mask_train[:, 0, :, :])
            loss = torch.multiply(loss, cfg.mask_train[:, 1, :, :])
            loss = loss.mean()
