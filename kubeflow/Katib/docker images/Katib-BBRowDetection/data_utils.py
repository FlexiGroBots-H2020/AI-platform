...
class AgroVisionDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return img.to(cfg.device), label.to(cfg.device), img_name
...
def data_loading(numpy_path,numpy_valid_path,binary,p_index,net_type,batch_size):
    shuffle_state = p_index
    train_loader = AgroVisionDataLoader(cfg.img_size, numpy_path, cfg.img_data_format, shuffle_state,
                                        batch_size, cfg.device, cfg.zscore, binary, cfg.dataset)
    
    valid_loader = AgroVisionDataLoader(cfg.img_size, numpy_valid_path, cfg.img_data_format, shuffle_state,
                                        batch_size, cfg.device, cfg.zscore, binary, cfg.dataset)
